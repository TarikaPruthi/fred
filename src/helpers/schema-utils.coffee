...
export toFhir = (decorated, validate) ->
	errCount = 0
...
export toBundle = (resources, pos, resource) ->
        resource

# getAllowedReferences: (schemaPath) ->

export getElementChildren = (profiles, schemaPath, excludePaths=[]) ->
...
export buildChildNode = (profiles, parentNodeType, schemaPath, fhirType) ->
...
export buildDisplayName = (schemaPath, fhirType) ->
...
export isResource = (profiles, data) ->
	if data.resourceType and
		profiles[data.resourceType]
			return true


export decorateFhirData = (profiles, data) ->
	nextId = 0
